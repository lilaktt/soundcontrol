...
public class SoundConfig {
    ...
    public static String getSoundGroup(String soundId) {
        if (soundId.startsWith("minecraft:entity.")) {
            ...
            if (parts.length >= 2) return "minecraft:entity." + parts[1];
        }
        if (soundId.startsWith("minecraft:block.")) {
            String[] parts = soundId.split("\\.");
            if (parts.length >= 2) return "minecraft:block." + parts[1];
        }
        return soundId;
    }

    private static float getSettingsVolume(SoundSettings s) { return s.muted ? 0.0f : s.volume; }
    ...
    public static float getVolumeModifier(String id) {
        if (SOUNDS.containsKey(id)) {
            ...
            if (!isDefault(s)) return getSettingsVolume(s);
        }
        ...
        if (id.contains(".break") && SOUNDS.containsKey("#global:break"))   return getSettingsVolume(SOUNDS.get("#global:break"));
        if (id.contains(".place") && SOUNDS.containsKey("#global:place"))   return getSettingsVolume(SOUNDS.get("#global:place"));
        if (id.contains(".step")  && SOUNDS.containsKey("#global:step"))    return getSettingsVolume(SOUNDS.get("#global:step"));
        if (id.contains(".hit")   && SOUNDS.containsKey("#global:hit"))     return getSettingsVolume(SOUNDS.get("#global:hit"));
        if (id.startsWith("minecraft:entity.")) {
            ...
            if (parts.length >= 2) {
                ...
                if (id.contains(".hurt")) {
                    if (isHostile  && SOUNDS.containsKey("#global:hostile_hurt"))  return getSettingsVolume(SOUNDS.get("#global:hostile_hurt"));
                    if (!isHostile && SOUNDS.containsKey("#global:passive_hurt"))  return getSettingsVolume(SOUNDS.get("#global:passive_hurt"));
                }
                if (id.contains(".ambient")) {
                    if (isHostile  && SOUNDS.containsKey("#global:hostile_ambient")) return getSettingsVolume(SOUNDS.get("#global:hostile_ambient"));
                    if (!isHostile && SOUNDS.containsKey("#global:passive_ambient")) return getSettingsVolume(SOUNDS.get("#global:passive_ambient"));
                }
            }
        }
        return 1.0f;

...
public class RecentSoundsPickerScreen extends Screen {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (keyCode == GLFW.GLFW_KEY_ESCAPE && this.shouldCloseOnEsc()) { this.close(); return true; }
    ...
    private static class RecentSoundEntry extends ElementListWidget.Entry<RecentSoundEntry> {
        ...
        public RecentSoundEntry(RecentSound sound, RecentSoundsPickerScreen parentScreen) {
            ...
            if (SoundConfig.SOUNDS.containsKey(sound.soundId)) {
                SoundConfig.SoundSettings ss = SoundConfig.SOUNDS.get(sound.soundId);
            ...
            this.muteButton = ButtonWidget.builder(
                ...
                button -> {
                    SoundConfig.SoundSettings s = SoundConfig.SOUNDS.computeIfAbsent(sid, k -> new SoundConfig.SoundSettings());
                    s.muted = !s.muted;
            ...
            this.resetButton = ButtonWidget.builder(Text.literal("\u27F2"), button -> {
                ...
                if (wasFav) { cur.volume = 1.0f; cur.muted = false; } else { SoundConfig.SOUNDS.remove(sid); }
                SoundConfig.save();
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            int rowLeft = this.parentScreen.soundList.getRowLeft();
            ...
            context.drawTextWithShadow(font, display, rowLeft + 2, y + 3, nameColor);
            if (showCopied) {
                int checkX = rowLeft + 2 + font.getWidth(display) + 4;
            ...
            context.drawTextWithShadow(font, timeStr, rowLeft + 2, y + 14, 0xFF888888);
            String posStr = String.format(java.util.Locale.US, "(%.0f, %.0f, %.0f)", sound.x, sound.y, sound.z);
            context.drawTextWithShadow(font, posStr, rowLeft + 30, y + 14, 0xFF666666);
            ...
            this.nameTooltipButton.setX(rowLeft + 2); this.nameTooltipButton.setY(y);
            this.nameTooltipButton.render(context, mouseX, mouseY, tickDelta);

            this.playButton.setX(rowLeft + 178); this.playButton.setY(y + 3);
            this.playButton.render(context, mouseX, mouseY, tickDelta);
            this.muteButton.setX(rowLeft + 200); this.muteButton.setY(y + 3);
            this.muteButton.render(context, mouseX, mouseY, tickDelta);
            this.volumeSlider.setX(rowLeft + 240); this.volumeSlider.setY(y + 3);
            this.volumeSlider.render(context, mouseX, mouseY, tickDelta);
            this.favoriteButton.setX(rowLeft + 322); this.favoriteButton.setY(y + 3);
            this.favoriteButton.render(context, mouseX, mouseY, tickDelta);
            this.resetButton.setX(rowLeft + 344); this.resetButton.setY(y + 3);
